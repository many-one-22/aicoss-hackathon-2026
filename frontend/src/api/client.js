...
async function fetchKosbert(text, region) {
  ...
  const timer = setTimeout(() => ctrl.abort(), 15000); // 8초 -> 15초: 콜드스타트(Cloud Run 슬립 후 재기동) 대비
  try {
    // cache: 'no-store' — 브라우저가 같은 검색어의 예전 응답을 캐시해서 재사용하는 바람에
    // 서버는 이미 고쳐졌는데도 화면엔 옛날 결과가 계속 뜨는 문제가 있었음.
...
export async function chatReply(text, loc = {}) {
  ...
  // [수정 이력] "노트북"처럼 백엔드가 understood:false를 정확히 계산해도, 여기서 그 값을
  // 안 보고 results.length만 체크해서 그냥 결과를 보여주던 버그가 있었음. 백엔드는 의도적으로
  // understood=false여도 결과를 지우지 않는 설계라(KoSBERT가 의미상 맞을 수도 있어서),
  // "진짜 이해 못 했다"는 판단은 여기 프론트에서 그 신호를 보고 내려야 한다.
  // (price 응답은 별개 종류라 이 체크 대상이 아님 — 위에서 먼저 분기됨)
  if (data && data.understood !== false) {
  ...
  if (!results.length) {
    return {
      messages: [
        // 말풍선을 둘로 나눈다 — ①왜 못 찾았는지 ②어떻게 다시 물어보면 되는지.
        // **…** 는 말풍선에서 굵게 렌더된다(Chat.jsx의 RichText).
        {
          who: 'bot',
          text: `음식이나 식당과 **관련 없는 단어**이거나, ${cityLabel} 근처에서 **알맞은 곳**을 찾지 못했어요.`,
        },
        {
          who: 'bot',
          text: '메뉴(예: 꼬막, 게장, 백반)나 방문 상황(예: 가족, 해장) 위주로 다시 입력해 주시겠어요?',
        },
      ],
      hits: [],
    };
